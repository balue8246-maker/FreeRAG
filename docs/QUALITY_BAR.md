...
FreeRAG app 不自动联网、不自动 OCR、不自动转写。MyRAG 运行时 skill 负责深挖和处理层写回；不同模型的 Vision / ASR 能力配置放在 `INSTALL_ADAPTERS.md`。
...
- 默认中文。
- `SKILL.md` 保持通用运行协议，不写死个人机器路径、某个模型或临时 workaround。
- `INSTALL_ADAPTERS.md` 只负责安装适配：判断模型能力，配置 Vision / OCR / ASR，并做真实图片/录音验证。
